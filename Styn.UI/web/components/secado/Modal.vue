    ...
    <div class="w-full max-w-lg bg-white rounded-lg shadow-lg">
      <div class="flex items-center justify-between p-4 border-b">
        <h2 class="text-xl font-semibold">
          {{ secado ? 'Editar Secado' : 'Nuevo Secado' }}
        </h2>
        <button
          @click="$emit('close')"
      ...
      <form @submit.prevent="handleSubmit" class="p-4">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div class="mb-4">
            <label for="tipoSecado" class="block mb-1 font-medium text-gray-700">Tipo de Secado</label>
            <select
              id="tipoSecado"
              v-model="formData.tipoSecado"
              class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              required
            >
          ...
          <div class="mb-4">
            <label for="tratamiento" class="block mb-1 font-medium text-gray-700">Tratamiento</label>
            <input
              id="tratamiento"
  ...
  mounted() {
    if (this.secado) {
      ...
      Object.keys(this.formData).forEach((key) => {
        if (key in this.secado) {
          if (key === "fecha") {
            // Formatear la fecha para el input date (YYYY-MM-DD)
            const date = new Date(this.secado[key])
            this.formData[key] = date.toISOString().split("T")[0]
          } else {
            this.formData[key] = this.secado[key]
          }
        }
      })
    }
